    ...
    def response(self):
        ...
        def do_get(url):
            return UserAgent().request(
                "get", url, headers=self.headers, preload_content=False
            )
...
def with_http_redirects(fun, url, *args, **kwargs):
    ...
    if not completed:
        raise Exception(
            "PUT {self.url} : status {response and response.status} : Too many redirects : {max_redirects}"
        )
    return response
